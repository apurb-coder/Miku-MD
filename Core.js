...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} dear 👋. I am ${global.BotName}, a bot developed by: Apurb to take your WhatsApp usage into next level.
    ...
    ©️ *${global.BotName}* All Rights Reserved by: *Apurb*
    `
...
//Some special chat replies
 ...
 let smallinput = budy.toLowerCase()
    ...
    if (smallinput=='bot') {
      reply (`Hello *${pushname}*, I am *${BotName}*, a WhatsApp bot made by *Apurb* and currently being hosted by *${OwnerName}*.  type  *${prefix}help* to get my full command list.`);
    }
...
case 'swm': case 'take': case 'stickerwm': case 'steal':{
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
if (!args.join(" ")) return reply(`Use command: -steal Miku|By: Apurb`)
const swn = args.join(" ")
...
case 'handsomecheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
				if (!text) return replay(`Tag Someone, Example : ${prefix + command} @Apurb`)
...
case 'charactercheck':
 ...
 case 'dare':
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "say i love the bot owner Apurb through voice note",
...
case 'truth':
    ...
    if (isBanChat) return reply(mess.bangc)
                 ...
                 "do you love the bot creator Apurb?",
...
I am *Miku Nakano*, a bot developed by *Apurb*.
...
case 'add':{     			
 ...
 case "tts":  case "texttospeech":  case "say": case "speak":{
    ...
    var replqr =  await getBuffer(`https://miku-qr--Apurb001.repl.co/`)
                               var qrbutton = [
